...
早期的神经网络，因无法处理两层以上的网络，都是浅层的。但是目前的深度学习，因为采取逐层预训练和全局微调策略，能够实现深层次的网络结构（实际中，通常需要10层以上的隐含层），取得较好的预测效果。但实际上训练一层的神经网络是非常困难的，通常包含非线性特征。

但是深度学习包括大量的计算，尤其是在训练阶段。深度学习并通过计算神经网络是提供，并通过后向反馈调整各层权值。

当前的深度学习网络中上百万的参数甚至是上千万的参数需要经过后向反馈来调整，并且深度学习需要大量的训练数据来确保分类和预测的准确性，也就意味着成几百万甚至是几千万的输入数据需要运行在前向或者后向反馈。因为深度学习由大量相同的并行神经元组成。这个并行化映射到GPU中，能够提供巨大的加速，如下图所示。这里使用NVIDIA公司提供的深度学习平台：CAFFE。 实验已表明，当训练“reference imagenet”时， 并行计算能够获得10倍于intel ivyBridge CPU的加速。
