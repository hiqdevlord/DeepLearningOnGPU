...
早期的神经网络都是浅层的，但是目前的深度学习都是深层次的，通常需要10层以上的网络，但实际上查过一层的神经网络的训练是非常困难的，通常包含非线性特征。

但是深度学习通常包括大量的计算，尤其是在训练阶段。深度学习
并通过计算神经网络是提供，并通过后向反馈调整各层权值。

当前的深度学习网络和需要上百万的参数甚至是上千万的数据需要经过后向反馈来调整，进一步，并且深度血虚需要大量的训练数据来首先分类和预测的准确性，也就意味着成几百万甚至是几千万的输入数据需要运行在前向或者后向反馈。因为深度学习由大量相同的并行的神经元组成。这个并行化映射到GPU中，能够提供提供巨大的加速，如下图所示。这里使用NVIDIA公司提供的深度学习平台CAFFE， 实验已经表明，相比intel ivyBridge CPU，并行计算其能够获得10倍的加速当训练“reference imagenet” 深度学习模型在NVIDIA Tesla K40 GPU，

NVIDIA cuDNN 是主要的深度学习GPU加速库。他提供深度学习中常用的应用。比如：卷积，池化，softmax，神经元激活（包括，sigmoid，矫正线性器，Hyperbolic tangent ），当然这些函数都支持前向、后向传播，cuDNN主要是在以矩阵相乘的方面擅长，cuDNN特性定数据输出，支持可伸缩的维度定制， dimension ordering, striding and subregions for 
对于四维张量。这些可伸缩性允许神经元积分，并且能够避免输入输出变换。
